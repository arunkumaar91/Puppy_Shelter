...
# Adding Shelters List

shelter1 = Shelter(name = "Texas Animal Services", address = "2304 Bay Area Ave", city = "Austin", state = "Texas", zipcode = "77055", website = "txanim.org")
session.add(shelter1)

shelter2 = Shelter(name = "Los Angeles Dog Rescue", address = "150 El Darado Blvd", city = "Los Angles", state = "California", zipcode = "", website = "LArescue.org")
session.add(shelter2)

shelter3 = Shelter(name = "Oakland Animal Services", address = "1101 29th Ave", city = "Oakland", state = "California", zipcode = "94601", website = "oaklandanimalservices.org")
session.add(shelter3)

shelter4 = Shelter(name = "San Francisco SPCA Mission Adoption Center", address = "250 Florida St", city = "San Francisco", state = "California", zipcode = "94103", website = "sfspca.org")
session.add(shelter4)

shelter5 = Shelter(name = "Wonder Dog Rescue", address= "2926 16th Street", city = "San Francisco", state = "California" , zipcode = "94103", website = "http://wonderdogrescue.org")
...
# Function to create random age for puppies.
def CreateRandomAge():
...
# Function to create random weight for puppies
def CreateRandomWeight():
    return random.uniform(1.0, 40.0)

for i,x in enumerate(male_names):
    new_puppy = Puppy(name = x, gender = "male", dateOfBirth = CreateRandomAge(), picture = random.choice(puppy_images), shelterId = randint(1,5), weight = CreateRandomWeight())
    session.add(new_puppy)
    session.commit()

for i,x in enumerate(female_names):
    new_puppy = Puppy(name = x, gender = "female", dateOfBirth = CreateRandomAge(), picture = random.choice(puppy_images), shelterId = randint(1,5), weight = CreateRandomWeight())
    session.add(new_puppy)
    session.commit()

print("Added Puppies List!")
